   ...
   <section id="details">
    <div id="details-wrapper">
      <div>
        ...
        <p id="details-title">${items.item}</p>
      </div>
      <div id="info-wrapper">
        <div id="details-description">
          <p class="details-price">Price: €${items.price}</p>
          <p class="details-availability">${items.availability}</p>
          <p class="type">Type: ${items.type}</p>
          <p id="item-description">${items.description}</p>
        </div>
...
const getDetails = (detailsId) => {
    return fetch(`http://localhost:3030/data/cyberpunk/${detailsId}`)
        .then(res => res.json())
